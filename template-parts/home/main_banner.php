<section class="Main_slider">
    <div class="w-full relative 2xl:h-[705px] md:h-[605px] h-[360px] overflow-hidden">
        <iframe
            class="absolute top-1/2 left-1/2 sm:w-[120vw] sm:h-[120vh] w-[220vw] h-[220vh] -translate-x-1/2 -translate-y-1/2"
            src="https://www.youtube.com/embed/dfViGz8LoZE?autoplay=1&mute=1&controls=0&loop=1&playlist=dfViGz8LoZE"
            allow="autoplay; fullscreen" frameborder="0">
        </iframe>
        <div class="video_overlay absolute h-full w-full bg-black/30 flex items-center">
            <div class="relative z-10 container mx-auto px-4">
                <div class="md:w-1/2 w-full">
                    <h1 class="md:text-5xl text-3xl font-semibold text-white mb-3">
                        Experience the Finest Coffee Blends
                    </h1>

                    <p class="text-white md:text-2xl text-lg font-medium mb-8">
                        Hand-selected beans from the world's best plantations, roasted to perfection for your daily
                        brew.
                    </p>

                    <a href="#"
                        class="text-sm font-medium px-8 py-3.5 border border-secondary text-white hover:text-coff_black bg-secondary hover:bg-primary hover:border-primary">
                        Shop Now <i class="fa fa-chevron-right"></i>
                    </a>
                </div>
            </div>
        </div>
    </div>
</section>